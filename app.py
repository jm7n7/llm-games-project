...
def draw_chess_board_pil():
    """Renders the chessboard using the Pillow library."""
    ...
    COLOR_BORDER = "#3C3A38"
    COLOR_COORD = "#E2E1E1"
    COLOR_SELECTED = (100, 149, 237, 178) # Cornflower Blue with 70% opacity
    COLOR_VALID_MOVE = (144, 238, 144, 153) # Light Green with 60% opacity
    COLOR_CHECK = (255, 0, 0, 178) # Red with 70% opacity
    ...
    # --- Load a font ---
    try:
        # Use a common font that supports chess characters well
    ...
    for r in range(8):
        for c in range(8):
            ...
            if piece:
                text_color = "#FFFFFF" if piece.color == 'white' else "#000000"
                text_width, text_height = font.getbbox(piece.symbol)[2:]
                draw.text(
                    (x0 + (SQUARE_SIZE - text_width) / 2, y0 + (SQUARE_SIZE - text_height) / 2 - 5), 
                    piece.symbol, 
                    font=font, 
                    fill=text_color
                )
    ...
    if king_pos and game.is_in_check(game.turn):
        r, c = king_pos
        x0, y0 = BORDER_SIZE + c * SQUARE_SIZE, BORDER_SIZE + r * SQUARE_SIZE
        # Create a temporary transparent overlay for the check color
        check_overlay = Image.new("RGBA", (SQUARE_SIZE, SQUARE_SIZE), (0,0,0,0))
        check_draw = ImageDraw.Draw(check_overlay)
        check_draw.rectangle([0, 0, SQUARE_SIZE, SQUARE_SIZE], fill=COLOR_CHECK)
        img.paste(check_overlay, (x0, y0), check_overlay)


    # --- Highlight selected square and valid moves ---
    if st.session_state.selected_square:
        # Create a temporary transparent overlay for highlights
        ...
        if selected_piece and selected_piece.color == game.turn:
            all_moves = selected_piece.get_valid_moves(board)
            valid_moves = [m for m in all_moves if not game.move_puts_king_in_check(st.session_state.selected_square, m)]
            for move_r, move_c in valid_moves:
                mx, my = BORDER_SIZE + move_c * SQUARE_SIZE, BORDER_SIZE + move_r * SQUARE_SIZE
                # Draw a circle for valid moves
    ...
    # --- Draw coordinates ---
    # We need a new Draw object because the image object was replaced by alpha_composite
    final_draw = ImageDraw.Draw(img)
    files = "abcdefgh"
    for i in range(8):
        # Files (a-h)
        final_draw.text(
            (BORDER_SIZE + i * SQUARE_SIZE + SQUARE_SIZE / 2, IMG_SIZE - BORDER_SIZE + 5), 
            files[i], font=coord_font, fill=COLOR_COORD, anchor="ms"
        )
        # Ranks (1-8)
        final_draw.text(
            (BORDER_SIZE - 10, BORDER_SIZE + i * SQUARE_SIZE + SQUARE_SIZE / 2),
            str(8 - i), font=coord_font, fill=COLOR_COORD, anchor="rm"
        )
        
    # Convert back to RGB for compatibility with streamlit_image_coordinates if needed,
    # but it often handles RGBA fine. Let's return RGBA first.
    return img

def handle_chess_click(coords):
    """Handles logic for updating game state when a square is clicked."""
    SQUARE_SIZE = 80
    BORDER_SIZE = 25
    ...
    if selected_piece:
        start_pos = st.session_state.selected_square
        success, message = game.make_move(start_pos, pos)
...
if game_selection == "Home":
    ...
    st.header("Chess (Built from Scratch)")
    ...
    with col1:
        ...
        board_image = draw_chess_board_pil()
        
        # Use streamlit-image-coordinates to get click events
        value = streamlit_image_coordinates(board_image, key="chess_board")
        
        # If the user clicked and it's a new click, handle the move
